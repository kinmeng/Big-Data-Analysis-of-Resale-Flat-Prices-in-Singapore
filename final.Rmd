...
```{r}
df |> 
  ml_linear_regression(formula = resale_price ~ floor_area_sqm + remaining_lease_std + dist_to_central + dist_nearest_mall + dist_nearest_mrt + storey_range) |>
  summary()
```
```{r}
library(sparklyr)
...
pipeline <- ml_pipeline(sc) |>
  ...
  ft_vector_assembler(
    input_cols = c("floor_area_sqm", "remaining_lease_std", "dist_to_central", "dist_nearest_mall", "dist_nearest_mrt", "flat_model_index", "storey_range_index"),
    output_col = "features"
  ) |>
  ft_standard_scaler(
    input_col = "features",
